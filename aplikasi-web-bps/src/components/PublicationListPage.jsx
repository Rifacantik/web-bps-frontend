...
export default function PublicationListPage() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Apakah Anda yakin ingin menghapus publikasi ini?")) {
      try {
        await deletePublication(id);  // Menghapus publikasi dari state dan memanggil API untuk menghapus data
      } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <header className="mb-8 text-center md:text-left">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">Daftar Publikasi BPS Provinsi Sulawesi Tengah</h1>
              ...
              <tr key={pub.id} className="bg-white border-b hover:bg-gray-50 transition-colors duration-200">
                ...
                <td className="px-6 py-4 text-center">
                  <div className="flex justify-center items-center space-x-2"> 
                    ...
                    <button
                      className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs font-semibold mx-2"
